...
const Services = () => {
    return (
        ...
        id='services' className='w-full px-[12%] py-10 scroll-mt-20'>
            ...
            transition={{duration: 0.5, delay: 0.7}}  className='text-center max-w-2xl mx-auto mt-5 mb-12 font-Ovo'>I am a Full stack Developer from Mumbai, Internship Experience of 1 Year in Multiple
                Companies like Heuristic Tech, Devlancers.
            </motion.p>
            <motion.div initial={{opacity: 0}} whileInView={{opacity: 1}} 
            transition={{duration: 0.6, delay: 0.9}}  className='grid grid-cols-auto gap-6 my-10'>
                {serviceData.map(({ icon, title, description, link }, index) => (
                    ...
                    hover:-translate-y-1 duration-500 dark:hover:bg-darkHover dark:hover:shadow-white'>
                        ...
                        <a href={link} className='flex items-center gap-2 text-sm mt-5'>Read More
                            <Image src={assets.right_arrow} className='w-4' alt='right arrow' /></a>
                    </motion.div>
                ))}
            </motion.div>
        </motion.div>
